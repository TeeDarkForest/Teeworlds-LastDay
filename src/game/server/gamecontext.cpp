...
void CGameContext::ConMake(IConsole::IResult *pResult, void *pUserData)
{
	...
	if(pResult->NumArguments()>0)
	{
		for(int i=WEAPON_SHOTGUN;i < WEAPON_NINJA;i++)
		{
			if(str_comp(pResult->GetString(0), pSelf->GetWeaponName(i)) == 0)
			{
				...
				{
					pSelf->SendChatTarget(ClientID, _("You need {int:Num} resource to make this."), 
						"Num", &NeedResource, NULL);
					return;
				}
			}
		}
	...
	pSelf->SendChatTarget(ClientID, 
		_("In this version you only can make: Shotgun, Grenade, Rifle"), NULL);
	return;
}
...
void CGameContext::OnZombie(int ClientID, int Zomb)
{
	...
	m_apPlayers[ClientID] = new(ClientID) CPlayer(this, ClientID, Zomb);
...
{
	int NumZombies=0;
	for(int i = MAX_CLIENTS-g_Config.m_LdMaxZombieNum-1;i<MAX_CLIENTS;i++)
	{
		if(m_apPlayers[i])
			if(m_apPlayers[i]->GetCharacter())
				if(m_apPlayers[i]->GetCharacter()->IsAlive())
					if(m_apPlayers[i]->IsZomb())
						NumZombies++;
	}
	return NumZombies;
}

void CGameContext::OnZombieKill(int ClientID, vec2 Pos)
{
	if(m_apPlayers[ClientID])
	{
		delete m_apPlayers[ClientID];
...
const char *CGameContext::GetWeaponName(int Weapon)
{
	switch (Weapon)
	{
		case WEAPON_HAMMER: return "Hammer";break;
		case WEAPON_GUN: return "Gun";break;
		case WEAPON_SHOTGUN: return "Shotgun";break;
		case WEAPON_GRENADE: return "Grenade";break;
		case WEAPON_RIFLE: return "Rifle";break;
	}
}

const char *CGameContext::GetResourceName(int Type)
{
	switch (Type)
	{
...
void CGameContext::SendKillMsg(int Killer, int Victim, int Weapon)
{
	...
	switch (Rand)
	{
		case 0: Msg.append("'{str:Killer}' killed '{str:Victim}'");break;
		case 1: Msg.append("'{str:Killer}' say 'Goodbye' to '{str:Victim}'");break;
		case 2: Msg.append("'{str:Killer}' say 'Hijack' to '{str:Victim}'");break;
	}
	SendChatTarget(-1, _(Msg.c_str()), "Killer", Server()->ClientName(Killer),
		"Victim", Server()->ClientName(Victim), NULL);
}
/*  Last Day End  */