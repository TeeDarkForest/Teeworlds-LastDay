...
void IGameController::Tick()
{
	...
	if(g_Config.m_SvInactiveKickTime > 0)
	{
		...
		#endif
			if(GameServer()->m_apPlayers[i] && !GameServer()->m_apPlayers[i]->IsZomb() && GameServer()->m_apPlayers[i]->GetTeam() != TEAM_SPECTATORS && !Server()->IsAuthed(i))
			{
...
{
	for(int i = 0; i < g_Config.m_LdMaxZombieNum; i++)//...
	{
		if(!GameServer()->m_apPlayers[MAX_CLIENTS-i-1])//Check if the CID is free
		{
			int Random = RandZomb();

			if(GameServer()->NumZombiesAlive() >= g_Config.m_LdMaxZombieNum)
				break;
			
			GameServer()->OnZombie(MAX_CLIENTS-i-1, Random);//Create a Zombie Finally
			m_Zombie[Random]--;
		}
	}
}
...
{
	int randnum = random_int(0, 100);
	if(randnum < 20)
	{
		return 0;
	}else if(random_int(0, 100) < 80)
	{
		return 1;
	}else return 2;
}
