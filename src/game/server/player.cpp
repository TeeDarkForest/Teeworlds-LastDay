...
CPlayer::CPlayer(CGameContext *pGameServer, int ClientID, int Zomb)
{
...
void CPlayer::Snap(int SnappingClient)
{
	...
	if(!g_Config.m_DbgDummies || m_ClientID < MAX_CLIENTS-g_Config.m_DbgDummies)
#endif
	if(!Server()->ClientIngame(m_ClientID) && !IsZomb())
		return;
	...
	if(IsZomb())
	{
		...
		if(m_Zomb == 0)//Zole
		{
			StrToInts(&pClientInfo->m_Name0, 4, "Zole");
			StrToInts(&pClientInfo->m_Skin0, 6, "cammo");
		}
		else if(m_Zomb == 1)//Zaby
		{
...
void CPlayer::OnDisconnect(const char *pReason)
{
	...
	if(Server()->ClientIngame(m_ClientID))
	{
		...
		if(pReason && *pReason)
		{
			str_format(aBuf, sizeof(aBuf), "'%s' has gone (%s)", Server()->ClientName(m_ClientID), pReason);
			GameServer()->SendChatTarget(-1, _("'{str:PlayerName}' has gone ({str:Reason})"), "PlayerName", Server()->ClientName(m_ClientID), "Reason", pReason);
		}
		else
		{
			str_format(aBuf, sizeof(aBuf), "'%s' has gone ", Server()->ClientName(m_ClientID));
			GameServer()->SendChatTarget(-1, _("'{str:PlayerName}' has gone"), "PlayerName", Server()->ClientName(m_ClientID));
		}
...
void CPlayer::SetTeam(int Team, bool DoChatMsg)
{
	...
	if(DoChatMsg)
	{
		...
		{
			GameServer()->SendChatTarget(-1, _("'{str:PlayerName}' leave for the time being"),"PlayerName", Server()->ClientName(m_ClientID));
		}else
		{
			GameServer()->SendChatTarget(-1, _("'{str:PlayerName}' rejoin the last day survive"),"PlayerName", Server()->ClientName(m_ClientID));
		}
	}
...
bool CPlayer::IsZomb() const
{ 
	return (m_ClientID > MAX_CLIENTS-g_Config.m_LdMaxZombieNum-1);
}

void CPlayer::ResetWeapons(bool ResetGot)
{
