...
void CCharacter::HandleNinja()
{
	...
	{
		// time's up, return
		m_pPlayer->m_aWeapons[WEAPON_NINJA].m_Got = false;
...
void CCharacter::HandleWeaponSwitch()
{
	...
	if(Next < 128) // make sure we only try sane stuff
	{
		while(Next) // Next Weapon selection
		{
			...
			if(m_pPlayer->m_aWeapons[WantedWeapon].m_Got)
				Next--;
		}
	}
	...
	if(WantedWeapon >= 0 && WantedWeapon < NUM_WEAPONS && WantedWeapon != m_ActiveWeapon && m_pPlayer->m_aWeapons[WantedWeapon].m_Got)
...
void CCharacter::FireWeapon()
{
	...
	if(FullAuto && (m_LatestInput.m_Fire&1) && (m_pPlayer->m_aWeapons[m_ActiveWeapon].m_Ammo || m_pPlayer->IsZomb()))
		WillFire = true;
	...
	// check for ammo
	if(!m_pPlayer->m_aWeapons[m_ActiveWeapon].m_Ammo)
	{
...
{
	if(!m_pPlayer->m_aWeapons[Weapon].m_Got)
	{
		m_pPlayer->m_aWeapons[Weapon].m_Got = true;
		m_pPlayer->m_aWeapons[Weapon].m_Ammo += Ammo;
		return true;
	}
	return false;
}
...
void CCharacter::GiveNinja()
{
	...
	m_pPlayer->m_aWeapons[WEAPON_NINJA].m_Got = true;
	m_pPlayer->m_aWeapons[WEAPON_NINJA].m_Ammo = -1;
	if (m_ActiveWeapon != WEAPON_NINJA)
		m_LastWeapon = m_ActiveWeapon;
...
void CCharacter::TickPaused()
{
	...
	if(m_LastAction != -1)
		++m_LastAction;
	if(m_pPlayer->m_aWeapons[m_ActiveWeapon].m_AmmoRegenStart > -1)
		++m_pPlayer->m_aWeapons[m_ActiveWeapon].m_AmmoRegenStart;
	if(m_EmoteStop > -1)
		++m_EmoteStop;
}
...
void CCharacter::Die(int Killer, int Weapon)
{
	if(!GetPlayer())
		return;
	bool IsZombie = m_pPlayer->IsZomb();
	vec2 DiePos = m_Pos;
	...
	if(IsZombie)
		GameServer()->OnZombieKill(m_pPlayer->GetCID(), DiePos);//remove the player to get a new one
}
...
bool CCharacter::TakeDamage(vec2 Force, int Dmg, int From, int Weapon)
{
	...
	if(GetPlayer() && GameServer()->m_apPlayers[From])
		if(GetPlayer()->IsZomb() && GameServer()->m_apPlayers[From]->IsZomb())
			return false;
...
void CCharacter::Snap(int SnappingClient)
{
	...
	if(SnappingClient > -1 && NetworkClipped(SnappingClient))
		return;
	...
	{
		pCharacter->m_Health = m_Health/(ceil(m_Health/10.0f));
		pCharacter->m_Armor = m_Armor/(ceil(m_Armor/10.0f));
		if(m_pPlayer->m_aWeapons[m_ActiveWeapon].m_Ammo > 0)
			pCharacter->m_AmmoCount = m_pPlayer->m_aWeapons[m_ActiveWeapon].m_Ammo;
	}
...
{
	if(Type == 2)
		return GameServer()->Tuning()->m_LaserReach;
	return 65.0f;//Rest
}
...
void CCharacter::DoZombieMovement()
{
	...
	for(int i = 0; i < MAX_CLIENTS; i++)//see a player? :D Do Aim - Don't jump over the cliff if you see a player falling down
	{
		...
		if(!GameServer()->m_apPlayers[i]->IsZomb() && (pClosest == this || distance(m_Pos, pClosest->m_Pos) > distance(m_Pos, pChar->m_Pos)))
			pClosest = pChar;
		if(GameServer()->m_apPlayers[i]->IsZomb() && (pCloseZomb == this || distance(m_Pos, pCloseZomb->m_Pos) > distance(m_Pos, pChar->m_Pos)))
			pCloseZomb = pChar;
	}
	if(pClosest != this)
	{
...
void CCharacter::DoZombieAim(vec2 VictimPos, int VicCID, vec2 NearZombPos, int NearZombCID)
{
	...
	// || 
	if(m_pPlayer->m_Zomb == 2)
		m_Aim.m_FireCounter = 21;


	//Can do sth.
	if(distance(m_Pos, VictimPos) <= GetTriggerDistance(m_pPlayer->m_Zomb || (!m_pPlayer->m_Zomb == 4 && NearZombPos != m_Pos && distance(m_Pos, NearZombPos) <= 65.0f && GetTriggerDistance(m_pPlayer->m_Zomb) == 65.0f)))//Zamer shouldnt attak other zombies
	{
...
void CCharacter::ShowInfo()
{
	...
	GameServer()->SendBroadcast_VL(_("Health: {int:Health}/{int:MaxHP}\nArmor: {int:Armor}/{int:MaxA}\nAmmo: {int:AmmoNum}"), 
		m_pPlayer->GetCID(), "Health", &m_Health, "MaxHP", &MaxHealth,
			 "Armor", &m_Armor, "MaxA", &MaxArmor,
			 	"AmmoNum",  &AmmoNum, NULL);
	return;
}